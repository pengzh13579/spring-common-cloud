...
var easyuiErrorFunction = function (XMLHttpRequest) {
  ...
  try {
    var emsg = XMLHttpRequest.responseText.substring(
        XMLHttpRequest.responseText.indexOf('错误描述'),
        XMLHttpRequest.responseText.indexOf('错误信息'));
        ...
        if (init == false) {
          for (var j = 0; j < cols.length; j++) {
            if (cols[j].field == fields[i] && cols[j].visible != false) {
              ...
              $('<div iconCls="icon-empty" field="' + fields[i] + '"/>').html(
                  fildOption.title).appendTo(tmenu);
            }
          }
        }
...
$.fn.tree.defaults.loadFilter = function (data, parent) {
  ...
  if (opt.parentField) {
    ...
    for (i = 0, l = data.length; i < l; i++) {
      if (tmpMap[data[i][parentField]] && data[i][idFiled]
          != data[i][parentField]) {
