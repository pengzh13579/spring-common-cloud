...
public class ExcelExportUtils {
    ...
    /**
     * 功能: 导出为Excel工作簿 参数: sheetName[工作簿中的一张工作表的名称] 参数: titleName[表格的标题名称] 参数: headers[表格每一列的列名] 参数: dataSet[要导出的数据源]
     * 参数: resultUrl[导出的excel文件地址] 参数: pattern[时间类型数据的格式]
     */
    ...
    private static void doExportExcel(String sheetName, String titleName, String[] headers, Collection<?> dataSet,
        ...
        for (short i = 0; i < headers.length; i++) {
            ...
            @SuppressWarnings ("deprecation") HSSFCell cell = row.createCell(i);
        ...
        while (it.hasNext()) {
            ...
            for (short i = 0; i < fields.length; i++) {
                ...
                try {
                    @SuppressWarnings ("rawtypes") Class tCls = t.getClass();
                    @SuppressWarnings ("unchecked") Method getMethod = tCls.getMethod(getMethodName, new Class[] {});
